Catch-N-Match
1. 프로젝트 개요
GA4 이벤트 데이터를 기반으로 사용자 행동을 분석하고, 구매 확률 예측 및 이탈 가능성 예측 결과를 생성하는 데이터/ML 파이프라인 프로젝트입니다.
...
4. Docker 컨테이너 실행
4-1. 컨테이너 빌드 및 실행 docker compose -f docker/compose.yml up -d --build 4-2. 컨테이너 상태 확인 docker ps
...
컨테이너 내부에서 확인 docker exec -it airflow-scheduler bash -lc "ls -al /app/output" 결과 파일 미리보기 docker exec -it airflow-scheduler bash -lc "head -n 5 /app/output/t7_prediction_result.csv"
...
ExternalTaskSensor로 DAG1 merge_csv_files 완료 대기 (최대 2시간)
...
현재 시각이 end_date를 초과하면 task skip → DAG success with no tasks 발생
실패 동작
실패 위치	동작
...
재시작 절차
docker compose up 이후 시간이 지나 end_date가 만료되면 DAG가 실행되지 않는다. 완전 재시작 시 .env의 AIRFLOW_DEPLOY_DATETIME을 up -d 직전 현재 KST 시각으로 반드시 업데이트할 것.
...
.env의 AIRFLOW_DEPLOY_DATETIME 형식: YYYY-MM-DDTHH:MM:SS (KST 기준)
...
GCP 서비스 계정 키 파일을 credentials/service_account.json에 위치시킬 것.
